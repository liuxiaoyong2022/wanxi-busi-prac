      ...
      <a-table
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <act-apply-btn @success="loadData" :data-id="record.id"
                         :variables="{ assigneeList:[],
                         total:record.age,
                         user_type: record.sex,
                         applyUserId:record.name,
                         assigneeUserIdList:['admin','jeecg','king'],
                         type:record.sex,
                          // dueDate:new Date((new date()).setMinutes((new date()).getMinutes()+5)),
                          dueDate: getTimeOut(180),
                         overTimeSendLimit:getTimeOut(60),
                         taskTimeout:0,
                         overTimeLimit:getTimeOut(90)}">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getTimeOut(delaySecond ){
    var currentTime=new Date();
    // var dueDate1=new Date(currentTime.setMinutes(currentTime.getMinutes()+5));
    // alert("dueDate:"+dueDate);
   return  new Date(currentTime.setSeconds(currentTime.getSeconds()+delaySecond));
  }
    }
  }
</script>
